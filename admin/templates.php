...
// the template functions can be found in staff-directory/functions.php
function staff_directory_templates() {
	...
	if(isset($_POST['staff-index-html'])){
		$sql = "UPDATE  $staff_directory_templates SET  `template_code` =  '" . $_POST['staff-index-html'] . "' WHERE  `template_name` = 'staff_index_html'";
		$wpdb->get_results($sql);
		
		$sql = "UPDATE  $staff_directory_templates SET  `template_code` =  '" . $_POST['staff-index-css'] . "' WHERE  `template_name` = 'staff_index_css'";
		$wpdb->get_results($sql);
		
		$output .= "<div id=\"message\" class=\"update fade\"><p>Settings Update</p></div>";
	}
	
	$staff_index_template = $wpdb->get_var("SELECT template_code FROM $staff_directory_templates WHERE template_name = 'staff_index_html';");
	$staff_index_css = $wpdb->get_var("SELECT template_code FROM $staff_directory_templates WHERE template_name = 'staff_index_css';");

    $output .= "<h2>Staff Directory Templates</h2>";
