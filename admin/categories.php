...
// Categories Page
function staff_directory_categories() {
	global $wpdb;
    ...
    if(!isset($_GET['action'])){
    	...
    	$addNewURL = get_bloginfo('wpurl') . "/wp-admin/admin.php?page=staff-directory-categories&action=addCategory";
    	...
    	foreach($categories as $category){
    		...
    		$editURL = get_bloginfo('wpurl') . "/wp-admin/admin.php?page=staff-directory-categories&action=edit&id=" . $category->cat_id;
			$deleteURL = get_bloginfo('wpurl') . "/wp-admin/admin.php?page=staff-directory-categories&action=delete&id=" . $category->cat_id;
    ...
    if(isset($_GET['action']) && $_GET['action'] == 'addCategory'){
    	...
    	if(isset($_POST['category-name'])){
    		...
    		$sql = "INSERT INTO $staff_directory_categories_table (cat_id, name) VALUES (NULL, '$category_name');";
    ...
    if(isset($_GET['action']) && $_GET['action'] == 'edit'){
    	...
    	$id = $_GET['id'];
		$staff_directory_categories_table = $wpdb->prefix . 'staff_directory_categories';
		$sql = "SELECT * FROM " . $staff_directory_categories_table . " WHERE cat_id = ". $id . ";";
    	...
    	if(isset($_POST['cat_name'])){
    		$sql = "UPDATE  " . $staff_directory_categories_table . " SET  `name` =  '" . $_POST['cat_name'] . "' WHERE  `cat_id` =  " . $id . ";";
    		$wpdb->get_results($sql);
    		$output .= "<h2>Edit Category</h2>";
    		$output .= "<div style=\"width:400px; padding:15px\">";
    ...
    if(isset($_GET['action']) && $_GET['action'] == 'delete'){
    	...
    	if(isset($_POST['confirm-category-delete']) && $_POST['confirm-category-delete'] == 'yes'){
    		$sql = "DELETE FROM `" . $staff_directory_categories_table . "` WHERE `cat_id` = " . $id . ";";
    		$wpdb->get_results($sql);
